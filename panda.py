...
class Panda(object):
    ...
    def __motif_data_to_matrix(self):
        ...
        print("Filter genes in motif data which are not in exp data")
        not_false_indices = [i for i, idx_value in enumerate(idx_genes) if idx_value != False]
        new_idx_tfs = [idx_tfs[idx] for idx in not_false_indices]
        new_idx_genes = [idx_genes[idx] for idx in not_false_indices]
        # idx = np.ravel_multi_index((idx_tfs, idx_genes), self.motif_matrix.shape)
        idx = np.ravel_multi_index((new_idx_tfs, new_idx_genes), self.motif_matrix.shape)

        # self.motif_matrix.ravel()[idx] = self.motif_data[2]
        self.motif_matrix.ravel()[idx] = [self.motif_data[2][index_] for index_ in not_false_indices]
        return None
    ...
    def __ppi_data_to_matrix(self):
        ...
        not_false_index = [i for i, idx_value in enumerate(idx_tf1) if ((idx_value != False) and (idx_tf2[i] != False))]
        new_idx_tf1 = [idx_tf1[idx] for idx in not_false_index]
        new_idx_tf2 = [idx_tf2[idx] for idx in not_false_index]
        idx = np.ravel_multi_index((new_idx_tf1, new_idx_tf2), self.ppi_matrix.shape)
        # self.ppi_matrix.ravel()[idx] = [self.ppi_data[2].tolist()[index_] for index_ in not_false_index]
        self.ppi_matrix.ravel()[idx] = [self.ppi_data[2][index_] for index_ in not_false_index]
        idx = np.ravel_multi_index((new_idx_tf2, new_idx_tf1), self.ppi_matrix.shape)
        # self.ppi_matrix.ravel()[idx] = [self.ppi_data[2].tolist()[index_] for index_ in not_false_index]
        self.ppi_matrix.ravel()[idx] = [self.ppi_data[2][index_] for index_ in not_false_index]
        return None
